...
class Oblig3Runner {
    ...
    static final Sorter[] ALGS1 = { new Insertion(), new Merge(), new Quick(), new Bubble() };
    // Put the sorting algorithms under test for part 2 here
    static final Sorter[] ALGS2 = { new Insertion(), new Merge(), new Quick(), new Bubble() };
    ...
    static String makeHeader(Sorter[] algs, int digits) {
        String headers = Arrays.stream(algs).map(alg -> alg.headerString()).collect(Collectors.joining(", "));
